...
const { evaluateSwingSignals, getIndexReturn, loadDaily } = require('../indicators/swingSignals');
const { openPosition, managePosition, openPositionCount, totalCapitalDeployed } = require('../trading/swingPositionManager');
...
async function scanForEntries(dateKey) {
  ...
  try {
    ...
    await safeSend(`⚠️ SWING SCAN FAILED\nCouldn't fetch candidate universe: ${e.message}`);
    return { scanned: 0, alerted: 0 };
  }

  const symbols = Object.keys(universe);
  console.log(`[swing] ${symbols.length} candidates passed the screener filter`);

  const indexReturn = await getIndexReturn();
  if (indexReturn === null) {
    console.warn('[swing] index return unavailable — RS LEADER signal disabled for this scan');
  }
  ...
  for (const result of found) {
    ...
    const labels = signals.map((s) => `${S.SWING_SIGNAL_EMOJI[s.label] || '🔔'} ${s.label}`).join(' + ');
    const details = signals.map((s) => `  ${S.SWING_SIGNAL_EMOJI[s.label] || '🔔'} ${s.detail}`).join('\n');

    const lines = [
      `📊 SWING SETUP — ${symbol}`,
      `💰 ₹${close.toFixed(2)}  |  ATR ₹${atr.toFixed(2)}  |  RSI ${rsi ? rsi.toFixed(0) : '-'}`,
      info?.sector ? `🏷 ${info.sector}` : null,
      info?.perf3Month !== undefined ? `📈 3M: ${info.perf3Month >= 0 ? '+' : ''}${info.perf3Month.toFixed(1)}%` : null,
      labels,
      details,
    ].filter(Boolean);

    // Only structural signals can define an entry stop
    const entrySignal = signals.find((s) => s.entry !== null && s.structuralStop !== null);

    if (entrySignal) {
      const outcome = openPosition(symbol, entrySignal.entry, atr, entrySignal.structuralStop, {
        dateKey,
      ...
      lines.push('ℹ️ Context signals only — no structural entry level, not sized');
    }
...
async function runSwingScan({ force = false } = {}) {
  ...
  try {
    ...
    eventStream.broadcast('scan_status', { mode: 'swing', status: 'started', dateKey });
    ...
    await pushClient.pushPositions(openList, state.totalRealizedPnl, state.counters, state.closedTrades);

    eventStream.broadcast('scan_status', {
      mode: 'swing', status: 'complete', dateKey, scanned, alerted, open: openList.length,
    });
...
async function tick() {
  try {
    ...
    if (!TEST_MODE) {
      ...
      } else if (min(time) >= min(S.SWING_SCAN_TIME) && state.lastScanDate !== dateKey) {
        await runSwingScan();
      }
    }
  } catch (e) {
...
function start() {
  ...
  if (open.length > 0) {
    console.log(`[swing] resumed tracking ${open.length} open position(s): ${open.map((p) => p.symbol).join(', ')}`);
  }
